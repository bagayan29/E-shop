<header class="header shop">
            ...
            <div class="row">
                <div class="col-12 d-flex justify-content-between align-items-center">
                    <ul class="list-main mb-0">
                        ...
                        <li><i class="ti-headphone-alt" style='color: blue'></i>@foreach($settings as $data)
                            {{$data->phone}} @endforeach</li>
                        <li><i class="ti-email" style='color: blue'></i> @foreach($settings as $data) {{$data->email}}
                            @endforeach</li>
                        <li><i class='fa fa-clock-o' style='color: blue'></i> @foreach($settings as $data)
                            {{ $data->timeAnddate }} @endforeach</li>
                    </ul>
                    <ul class="list-main mb-0">
                        <li><i class="ti-location-pin" style='color: blue'></i> <a
                                href="{{route('order.track')}}">Shipment Tracking</a></li>
                        @auth
                        @if(Auth::user()->role=='admin')
                        <li><i class="ti-user" style='color: blue'></i> <a href="{{route('admin')}}"
                                target="_blank">Admin Dashboard</a></li>
                        @else
                        <li><i class="ti-user" style='color: blue'></i> <a href="{{route('user')}}"
                                target="_blank">Profile</a></li>
                        @endif
                        <li><i class="bi bi-box-arrow-right" style='color: red'></i> <a
                                href="{{route('user.logout')}}">Logout</a></li>
                        @else
                        <li><i class="bi bi-box-arrow-left" style='color: blue '></i><a href="{{route('login.form')}}">Login
                                /</a> <a href="{{route('register.form')}}">Register</a></li>
                        @endauth
                    </ul>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-7 col-12">
                    <div class="search-bar-top">
                        <div class="search-bar" style="border-radius: 20px;">
                            ...
                            <form method="POST" action="{{route('product.search')}}">
                                @csrf
                                <input name="search" placeholder="Search Any Products Here" type="search">
                                <button class="btnn" type="submit" style="border-radius: 20px;"><i class="ti-search"></i></button>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <!-- Header Inner -->
    <div class="header-inner" style="background-color: gray;">
        <div class="container">
